...
export async function listPositions(
  ...
  const data = (await res.json().catch(() => undefined)) as PositionsListResponse | undefined;
  ...
  const items = rawItems.map((item) => normalizePosition(item)).filter((pos) => pos.id && pos.name);
