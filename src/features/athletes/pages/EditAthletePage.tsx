...
import { getAthleteById, updateAthlete, type AthleteListItem } from '../services/athleteService';
import {
...
export default function EditAthletePage() {
  ...
  React.useEffect(() => {
    ...
    const loadAthlete = async () => {
      ...
      try {
        ...
        if (!active) return;
        setAthlete(null);
        setLoadError(err instanceof Error ? err.message : 'Failed to load athlete.');
      } finally {
  ...
  const relationshipMenuOptions = React.useMemo(() => {
    ...
    if (!trimmedRelationship || relationshipOptions.includes(trimmedRelationship)) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to update athlete.';
