...
export function pickPrimarySkillMedia(media: SkillMedia[] | null | undefined): SkillMedia | null {
  if (!Array.isArray(media) || media.length === 0) return null;
  const sorted = [...media].sort(sortByPosition);
  return sorted[0] ?? null;
}

export function pickPrimarySkillVideo(media: SkillMedia[] | null | undefined): SkillMedia | null {
...
export function pickSkillThumbnailUrl(media: SkillMedia[] | null | undefined): string | null {
