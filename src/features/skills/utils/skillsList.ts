...
export const filterSkills = (skills: Skill[], filters: SkillFilters) => {
  ...
  return skills.filter((skill) => {
    const matchesQuery =
      ...
      normalizeText(
        Array.isArray(skill.coaching_points) ? skill.coaching_points.join(' ') : '',
      ).includes(q);

    const matchesCategory = filters.category === FILTER_ALL || skill.category === filters.category;

    return matchesQuery && matchesCategory;
  });
};

export const buildCategoryCounts = (skills: Skill[]): SkillCategoryCount[] => {
