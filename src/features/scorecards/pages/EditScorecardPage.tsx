...
export default function EditScorecardPage() {
  ...
  React.useEffect(() => {
    ...
    getScorecardTemplateDetail(templateId, { orgId })
      .then((detail) => {
        if (!active) return;
        const normalizedTemplate = detail.template ?? null;
        setTemplate((prev) => ({
          id: templateId,
        ...
        originalCategoryIdsRef.current = nextCategories
          .map((cat) => cat.id)
          .filter((catId) => isUuid(catId));
        originalSubskillIdsRef.current = nextSubskills
          .map((sub) => sub.id)
          .filter((subId) => isUuid(subId));

        const categoryMap: Record<string, ScorecardCategory> = {};
        nextCategories.forEach((cat) => {
          if (cat.id) categoryMap[cat.id] = cat;
        });
        originalCategoryMapRef.current = categoryMap;

        const subskillMap: Record<string, ScorecardSubskillRow> = {};
        nextSubskills.forEach((sub) => {
          if (sub.id) subskillMap[sub.id] = sub;
        });
        originalSubskillMapRef.current = subskillMap;
      })
      .catch((err: any) => {
        if (!active) return;
  ...
  const handleSave = async () => {
    ...
    try {
      const currentCategoryIds = new Set(
        categories.map((cat) => cat.id).filter((catId) => isUuid(catId)),
      );
      const removedCategoryIds = originalCategoryIdsRef.current.filter(
        (catId) => !currentCategoryIds.has(catId),
      );

      const changedCategoryIds = new Set<string>();
      categories.forEach((cat) => {
        if (!isUuid(cat.id)) return;
      ...
      const removeCategoryIds = Array.from(
        new Set([...removedCategoryIds, ...Array.from(changedCategoryIds)]),
      );

      const addCategories = categories.filter(
        (cat) => !isUuid(cat.id) || changedCategoryIds.has(cat.id),
      );
      ...
      const currentSubskillIds = new Set(
        subskills.map((sub) => sub.id).filter((subId) => isUuid(subId)),
      );
      const removedSubskillIds = originalSubskillIdsRef.current.filter(
        (subId) => !currentSubskillIds.has(subId),
      );

      const changedSubskillIds = new Set<string>();
      subskills.forEach((sub) => {
        if (!isUuid(sub.id)) return;
        ...
        if (
          ...
          changedSubskillIds.add(sub.id);
        }
      });

      const removeCategorySet = new Set(removeCategoryIds);
      const removeSubskillIds = originalSubskillIdsRef.current.filter((subId) => {
        const original = originalSubskillMapRef.current[subId];
        if (!original) return true;
        if (!currentSubskillIds.has(subId)) return true;
        if (changedSubskillIds.has(subId)) return true;
        if (removeCategorySet.has(original.category_id)) return true;
        return false;
      });

      const addSubskills = subskills
        .filter((sub) => {
          if (!isUuid(sub.category_id)) return false;
          if (removeCategorySet.has(sub.category_id)) return false;
          const isNew = !isUuid(sub.id);
