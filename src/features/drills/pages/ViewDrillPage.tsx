...
import { getDrilById, getDrillMediaPlay, type DrillItem } from '../services/drillsService';
...
export default function ViewDrillPage() {
  ...
  const primaryMedia = React.useMemo(() => pickPrimaryMedia(drill?.media ?? []), [drill?.media]);
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!active) return;
        setPlayError(err instanceof Error ? err.message : 'Failed to load drill video.');
        setPlayUrl(null);
      } finally {
  ...
  const playerLabel = drill
    ...
    : '';
  const ageLabel = drill ? formatRange(drill.min_age, drill.max_age, 'years', 'Any age') : '';
          ...
          <>
            <Stack spacing={1}>
              <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                ...
                {drill.is_archived && <Chip size="small" label="Archived" variant="outlined" />}
              </Stack>
              <Typography variant="body2" color="text.secondary">
                {drill.segment?.name ?? 'General'} {drill.level ? `• ${drill.level}` : ''}
              </Typography>
            </Stack>
            ...
            <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 } }}>
              ...
              <Grid container spacing={2}>
                ...
                <DetailItem
                  label="Duration"
                  value={Number.isFinite(drill.duration_min) ? `${drill.duration_min} min` : '—'}
                />
            ...
            <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 } }}>
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap">
                {drill.skill_tags.length > 0 ? (
                  drill.skill_tags.map((tag) => <Chip key={tag.id} label={tag.name} size="small" />)
                ) : (
                  <Typography variant="body2" color="text.secondary">
                    No tags assigned.
                  </Typography>
