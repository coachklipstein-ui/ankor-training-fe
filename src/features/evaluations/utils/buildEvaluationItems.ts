...
export function buildEvaluationItems({
  ...
  athletes.forEach((athlete) => {
    ...
    for (const [categoryId, rawCatScore] of Object.entries(categoryScoresForAthlete)) {
      ...
      subskills.forEach((sub) => {
        ...
        const explicitSubRating =
          subskillRatingsForCategory[subskillKey] ?? subskillRatingsForCategory[sub.id];
        const rawRating = explicitSubRating !== undefined ? explicitSubRating : catScore;
    ...
    for (const [categoryId, subskillsRatings] of Object.entries(subskillScoresForAthlete)) {
      ...
      subskills.forEach((sub) => {
        const subskillKey = sub.skill_id ?? sub.id;
        const rating = normalizeRating(subskillRatings[subskillKey] ?? subskillRatings[sub.id]);
        if (rating === null) return;
