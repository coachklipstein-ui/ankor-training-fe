...
type UseSkillsDialogArgs = {
  ...
  setSubskillsByCategory: React.Dispatch<React.SetStateAction<Record<string, ScorecardSubskill[]>>>;
  ...
  setSubskillEvaluations: React.Dispatch<React.SetStateAction<SubskillEvaluationsState>>;
  orgId: string | null;
};
...
export function useSkillsDialog({
  ...
  const [skillDialogCategory, setSkillDialogCategory] = React.useState<ScorecardCategory | null>(
    null,
  );
  const [skillDialogAthlete, setSkillDialogAthlete] = React.useState<string | null>(null);
  const [skillDialogSkills, setSkillDialogSkills] = React.useState<ScorecardSubskill[]>([]);
  ...
  const saveSkillsDialog = React.useCallback(() => {
    if (skillDialogAthlete && skillDialogCategory) {
      ...
      const baseline =
        normalizeRating(evaluations[skillDialogAthlete]?.[dialogCategoryId] ?? null) ?? null;
  ...
  const handleSkillRatingChange = React.useCallback((skillId: string, rating: number | null) => {
  ...
  }, [skillDialogOpen, skillDialogAthlete, skillDialogCategory, subskillEvaluations]);
