import { ensureValidSession, getOrgId, refreshAuthSession } from '../auth/authClient';
...
export async function apiFetch(url: string, options: ApiFetchOptions = {}): Promise<Response> {
  const { requireAuth = true, includeOrgId = true, orgId, ...init } = options;
  ...
  const finalUrl = includeOrgId && resolvedOrgId ? withOrgId(url, resolvedOrgId) : url;
